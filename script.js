...
import { renderShell, renderHeader, categories, mockPosts, categoryCard, postCard, statCard, bindPostInteractions } from './ui.js';
...
function renderHome(user){
  document.getElementById('mainContent').innerHTML = `
    ...
    <section class="section"><div class="section-title"><h2>Bài viết mới</h2><a href="post.html">Viết bài</a></div><div class="post-grid">${mockPosts.map(p=>postCard(p,user)).join('')}</div></section>
    <section class="section two-col"><div><h2>Bài nổi bật</h2>${mockPosts.filter(p=>p.featured).map(p=>postCard(p,user)).join('')}</div><div><h2>Được yêu thích</h2>${mockPosts.sort((a,b)=>b.likes-a.likes).slice(0,3).map(p=>postCard(p,user)).join('')}</div></section>
